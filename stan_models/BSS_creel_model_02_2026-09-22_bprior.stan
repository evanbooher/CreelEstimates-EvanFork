...
// diff against it to confirm.
//
// TWO-CHANNEL (G = 2): the model for a fishery-year with both bank and boat
// anglers interviewed -- fw_creel_bprior_2026_snohomish.Rmd and the generic
// fw_creel_bprior.Rmd. value_lognormal_mu_b / _sigma_b are vector[G], which
// matches the length-2 c(vehicle, trailer) vectors prep_inputs_bss_bprior()
// builds only when G = 2. A bank-only fishery-year (G = 1) uses
// BSS_creel_model_02_2026-09-22_univariate_bprior.stan instead, as the
// Stillaguamish 2026 render does. Restored 2026-09-24 from commit 9b1acee^,
// where it had been overwritten by that univariate rebuild. Kept as a SEPARATE dated file rather than
