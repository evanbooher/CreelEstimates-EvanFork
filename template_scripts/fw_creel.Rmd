...
keep_try <- function(label, expr) {
  ...
  if (inherits(out, "try-error")) {
    cli::cli_alert_warning(
      "{label} failed for {ecg} -- the fit is unaffected. {conditionMessage(attr(out, 'condition'))}"
    )
    return(NULL)
  }
  out
...
for(ecg in names(inputs_bss)) {
  ...
  if (inherits(ecg_fit, "try-error")) {
    cli::cli_alert_danger(
      "fit_bss FAILED for {ecg} -- skipping this catch group; any other group in this \
       fishery is unaffected. {conditionMessage(attr(ecg_fit, 'condition'))}"
    )
    rm(ecg_fit); gc()
    next
  }
  ...
  if (is.null(ecg_keep$draws)) {
    cli::cli_alert_danger("No draws for {ecg} -- skipping this catch group.")
    rm(ecg_fit); gc()
    next
  }
