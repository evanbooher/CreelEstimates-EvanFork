...
for(ecg in names(inputs_bss)) {
  ...
  ecg_keep <- list()
  # Each of these is a post-fit SUMMARY. The fit is already complete by the time
  # they run, and estimates_bss.rds is not written until the end of this chunk,
  # so any one of them throwing has always destroyed a finished fit -- which is
  # exactly what get_bss_overview() did at four chains. Individually guarded:
  # a summary that cannot be built costs that summary, nothing else.
  ...
  # Skipped under fit_only: a plot nothing will render, and one more thing that
  # can fail between a completed fit and it being written to disk.
  ecg_keep$density_plot <- if (isTRUE(params$fit_only)) NULL else
    create_bss_density_plot(ecg_keep$season_results, ecg)
...
combined_density_plot <- if (isTRUE(params$fit_only)) NULL else
  save_bss_density_plots(density_plots, outputs_folders)
