...
params:
  ...
  fishery_name: "Hoh winter steelhead 2023-24"
  est_date_start: "2023-12-01"
  est_date_end: "2024-03-31"
...
# Calculate tie-in (aka census) count expansion factor by section & angler_final
  ...
  (inputs_pe$paired_census_index_counts <- 
    prep_inputs_pe_paired_census_index_counts_aggregate(
      days = dwg$days,
      dwg_summarized = dwg_summ,
...
# Calculate census-corrected angler effort (mean angler hours) by event_date, angler_type, and section_num
  (inputs_pe$ang_hrs_daily_mean <- 
    prep_inputs_pe_ang_hrs_aggregate(
      days = dwg$days, 
      dwg_summarized = dwg_summ,
      interview_ang_per_object = inputs_pe$interview_ang_per_object,
...
```{r view_paired_census_index_counts, eval=FALSE, cache=params$enable_cache, dependson=c("establish_file_structure", "prep_inputs_pe")}
...
dwg_summ$interview |> group_by(est_cg) |> summarise(sum(fish_count)/sum(fishing_time_total))
```
...
save_draws <- TRUE  # Change to TRUE if you want to save draws
# ----------------------
...
### Comparison of BSS and PE estimates, interview reported catches by section, and interview CPUE by section and angler type

One key difference between the BSS and PE outputs is section 3 effort - the BSS effort estimates for section 3 (12.5k) appear more coherent with sections 1 (20k) and 2 (11.5k), while the PE effort estimate for section 3 is substantially lower at around 4.5k. 
...
```{r plot_bss_pe, cache=params$enable_cache, dependson="transform_estimates", cache.extra=param_hash}

creel_estimates$stratum_effort |> 
  mutate(period = forcats::fct_reorder(factor(period), min_event_date)) |>
  filter(estimate_type %in% c("est", "mean")) |>
  group_by(model_type, period, section_num, angler_final, est_cg) |> 
  summarise(weekly_effort_estimate = sum(value)) |> 
  ggplot(aes(period, weekly_effort_estimate, color = model_type)) +
  geom_line() +
  geom_point() +
  facet_wrap(.~angler_final + section_num) + 
  theme_bw()

creel_estimates$stratum_effort |> 
  mutate(period = forcats::fct_reorder(factor(period), min_event_date)) |>
  filter(estimate_type %in% c("est", "mean")) |>
  group_by(model_type, section_num, est_cg) |> 
  summarise(weekly_effort_estimate = round(sum(value), 0)) |> 
  arrange(model_type) |> 
  gt()


creel_estimates$stratum_catch |> 
  mutate(period = forcats::fct_reorder(factor(period), min_event_date)) |>
  filter(estimate_type %in% c("est", "mean")) |>
  group_by(model_type, period, section_num, angler_final, est_cg) |> 
  summarise(weekly_catch_estimate = sum(value)) |> 
  ggplot(aes(period, weekly_catch_estimate, color = model_type)) +
  geom_line() +
  geom_point() +
  facet_wrap(.~angler_final + section_num) + 
  theme_bw()
...
creel_estimates$stratum_catch |> 
  ...
  filter(estimate_type %in% c("est", "mean")) |>
  group_by(model_type, section_num, est_cg) |> 
  summarise(weekly_estimate = round(sum(value), 0)) |> 
  arrange(model_type) |> 
  gt()

dwg_summ$interview |> 
  group_by(section_num) |> 
  summarise(interview_total_catch = sum(fish_count)) |> 
  gt()

dwg_summ$interview |> 
  group_by(est_cg, angler_final, section_num) |> 
  summarise(CPUE = sum(fish_count)/sum(fishing_time_total)) |> 
  mutate(CPUE = round(CPUE,2)) |> 
  arrange(section_num, angler_final) |> 
  gt()
...
dwg_summ$interview |> 
  ...
  mutate(week = forcats::fct_reorder(factor(week), event_date)) |> 
  group_by(est_cg, week, angler_final, section_num) |>
  summarise(num_obs = n()) |> 
  ggplot(aes(week, num_obs, color = section_num)) +
  geom_line() +
  geom_point() +
  facet_wrap(.~angler_final + section_num) + 
  theme_bw()
...
```{r proportion_effort_sampled, cache=params$enable_cache, dependson=c("dwg_fetch", "prep_dwg_summ_shared_summary_objects", "estimates_pe"), cache.extra=param_hash}

est_pe_effort_sampled_by_interviews(dwg = dwg, dwg_summ = dwg_summ, estimates_pe = estimates_pe)
